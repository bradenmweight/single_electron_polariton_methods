...
BASIS_PHOTON = "Fock"
BASIS_ELECTRON = "K"
DATA_DIR = "data_AD_Fock10_K100_biggerLIMIT" #f"data_{HAM}_{BASIS_PHOTON}_{BASIS_ELECTRON}"
...
#EPol = np.zeros(( len(g_wc_list), 10000 ))
...
for g_wc_IND, g_wc in enumerate( g_wc_list ):
    ...
    #EPol[g_wc_IND,:] = np.loadtxt( f"../PF/data_PF_Fock200_NR2048_NM50/E_PF_Fock_R_gwc{np.round(g_wc,7)}_wc1.0324_Transition.dat" ) # Extract transition energies
...
# Save Data
np.savetxt(f"EPOL_{HAM}_{BASIS_PHOTON}_{BASIS_ELECTRON}_wc{np.round(wc,4)}_Transition.dat", EPol[:,:100])
np.savetxt(f"N_{HAM}_{BASIS_PHOTON}_{BASIS_ELECTRON}_wc{np.round(wc,4)}_Transition.dat", N_1[:,:100])
np.savetxt(f"N_{HAM}inPABasis_{BASIS_PHOTON}_{BASIS_ELECTRON}_wc{np.round(wc,4)}_Transition.dat", N_2[:,:100])
...
plt.savefig(f"{DATA_DIR}/g_wc_Scan_{HAM}Basis__NR{Nmatter}_NF{Nph}.jpg",dpi=300)
...
for state in plot_states:
    ...
    plt.scatter( g_wc_list, EPol[:,state], c=N_1[:,state],s=70,marker="o",edgecolor='none',cmap=plt.cm.hsv, vmin=0, vmax=4 )
plt.legend()
plt.colorbar()
#plt.xlim(0.01,100)
plt.xlim(0,2.5)
...
for state in plot_states:
    ...
    plt.scatter( g_wc_list, EPol[:,state], c=N_2[:,state],s=70,marker="o",edgecolor='none',cmap=plt.cm.hsv, vmin=0, vmax=4 )
plt.legend()
plt.colorbar()
#plt.xlim(0.01,100)
plt.xlim(0,2.5)
