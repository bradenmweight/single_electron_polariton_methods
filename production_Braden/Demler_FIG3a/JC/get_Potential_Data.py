#!/scratch/bweight/anaconda3/bin/python3
import numpy as np
...
def get_V_x__1Erfs():
    RGrid = np.linspace(-10,10,nR)
    r0 = 1  # Erf width --> Erf( x/r0 ) / x, r0 = Length (a.u.)
    Vx = np.zeros(( nR ))
    Vx += -10*erf( RGrid / r0 ) / RGrid
    if ( FIX_BOUNDARIES ):
        Vx = correct_Boundaries( Vx )
    return RGrid, Vx
...
def plot_wfns_X( E,U,Vx,RGrid ):
    ...
    plt.plot( RGrid, 5*U[:,0] + E[0], c="red", alpha=1.0, linewidth=3 )
    plt.plot( RGrid, 5*U[:,1] + E[1], c="blue", alpha=1.0, linewidth=3 )
    plt.plot( RGrid, -5*U[:,2] + E[2], c="green", alpha=1.0, linewidth=3 )
    #plt.plot( RGrid, U[:,3] + E[3], c="orange", alpha=1.0, linewidth=3 )
    #plt.plot( RGrid, U[:,4] + E[4], c="pink", alpha=1.0, linewidth=3 )
    #plt.plot( RGrid, U[:,5] + E[5], c="purple", alpha=1.0, linewidth=3 )
    plt.xlim(-2,2)
    plt.ylim(0,6)
...
def get_dipole_matrix( RGrid, U ): # NOT RIGOROUSLY TESTED. NEED TO IMPLEMENT IN K-SPACE AS WELL.
    ...
    for j in range( len(RGrid) ):
        for k in range( len(RGrid) ):
            ...
            MU[ j,k ] = -np.sum( U[:,j] * RGrid[:] * U[:,k] )# * dR ### dR SHOULD APPEAR. WHY IS IT WRONG ?
    np.savetxt( "Vx/MU.dat", MU )
    #plt.imshow( np.abs(MU[:8,:8]), origin='lower', vmin=0, vmax=1.0 )
    #plt.imshow( np.log( np.abs(MU[:8,:8]) + 0.001 ), origin='lower', vmin=-4, vmax=0 )
    plt.imshow( np.abs(MU[:8,:8]), origin='lower', norm=LogNorm(vmin=0.01, vmax=1) )
    plt.colorbar()
...
def main():
    ...
    #RGrid, Vx = get_V_x__1Erfs()
    RGrid, Vx = get_Demler_FIG3_a()
    #RGrid, Vx = get_Demler_FIG3_b()
    #RGrid, Vx = get_QHO()
