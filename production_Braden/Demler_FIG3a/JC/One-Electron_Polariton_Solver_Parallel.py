...
def get_Globals():
    ...
    HAM = "JC" # "AD", "PF", "JC" -- Solves the Asympotically Decoupled (AD), Pauli-Fierz (PF), Jaynes-Cummings [PF + RWA - DSE] (JC) Hamiltonians
    ...
    Nf  = 2  # Only used if calculating result in Fock basis
...
def get_H_Total_Fock_Real__Pauli_Fierz__Truncated( Had, MU, op_b, A0 ): # Enforce additional matter trunction
    ...
    NTrunc = 20 # Includes S0 and S1 for NTrunc = 2
...
def main_parallel():
    ...
    with mp.Pool(processes=NCPUS) as pool:
        if( HAM == 'AD' and BASIS_PHOTON == "Fock" and BASIS_ELECTRON == "K" ):
        ...
        elif( HAM == 'PF' and BASIS_PHOTON == "Fock" and BASIS_ELECTRON == "R" ):
