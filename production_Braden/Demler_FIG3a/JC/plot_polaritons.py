...
EPol_exact = np.zeros(( len(g_wc_list), NPol ))
...
for g_wc_IND, g_wc in enumerate( g_wc_list ):
    print (g_wc)
    #try:
    EPol_exact[g_wc_IND,:] = np.loadtxt( f"{DATA_DIR}/E_{HAM}_{BASIS_PHOTON}_{BASIS_ELECTRON}_gwc{np.round(g_wc,7)}_wc{np.round(wc,4)}.dat" ) # Extract energies
...
np.savetxt( f"{DATA_DIR}/plot_data_NR{Nmatter}_NF{Nph}.dat", np.c_[g_wc_list,EPol_exact[:,0],EPol_exact[:,1],EPol_exact[:,2],EPol_exact[:,3] ] )
np.savetxt( f"{DATA_DIR}/plot_data_NR{Nmatter}_NF{Nph}_Transition.dat", np.c_[g_wc_list,EPol[:,0],EPol[:,1],EPol[:,2],EPol[:,3] ] )
...
for state in plot_states:
    ...
    plt.scatter( g_wc_list, EPol[:,state], c=N_1[:,state],s=70,marker="o",edgecolor='none',cmap=plt.cm.hsv, vmin=0, vmax=1 )
    #plt.scatter( g_wc_list, EPol[:,state], c=N_1[:,state],s=70,marker="o",edgecolor='none',cmap=plt.cm.hsv, vmin=0, vmax=10 )
plt.legend()
plt.colorbar()
plt.xlim(0.0,2.5)
