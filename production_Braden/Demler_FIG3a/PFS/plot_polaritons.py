...
EPol = np.zeros(( len(g_wc_list), NKeep)) #NPol ))
N_1 = np.zeros(( len(g_wc_list), NKeep)) #NPol )) # Photon number in original basis
N_2 = np.zeros(( len(g_wc_list), NKeep)) #NPol )) # Photon number in p.A basis
...
for g_wc_IND, g_wc in enumerate( g_wc_list ):
    ...
    EPol[g_wc_IND,:] = np.loadtxt( f"{DATA_DIR}/E_{HAM}_{BASIS_PHOTON}_{BASIS_ELECTRON}_gwc{np.round(g_wc,7)}_wc{np.round(wc,4)}_NFock{Nph}_NM{Nmatter}_Transition.dat" )[:NKeep] # Extract transition energies
...
if ( len(EPol[0,:]) < 10 ):
    plot_states = np.arange( len(EPol[0,:]) )
else:
...
# plt.savefig(f"{DATA_DIR}/g_wc_Scan_NFock{Nph}_NM{Nmatter}.jpg",dpi=300)
