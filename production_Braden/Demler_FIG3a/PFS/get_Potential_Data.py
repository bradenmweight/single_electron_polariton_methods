...
def plot_wfns_X( E,U,Vx,RGrid ):
    ...
    plt.plot( RGrid, 10*U[:,4] + E[4], c="pink", alpha=1.0, linewidth=3 )
    plt.plot( RGrid, 10*U[:,5] + E[5], c="purple", alpha=1.0, linewidth=3 )
    plt.xlim(-10,10)
    plt.ylim(-12,1)
...
def get_dipole_matrix( RGrid, U ): # NOT RIGOROUSLY TESTED. NEED TO IMPLEMENT IN K-SPACE AS WELL.
    ...
    for j in range( len(RGrid) ):
        print(j)
        for k in range( j, len(RGrid) ):
             # Could do better integration. Maybe with scipy.
            MU[ j,k ] = -np.sum( U[:,j] * RGrid[:] * U[:,k] ) # Wavefunctions are normalized with dR already. No need to include again.
            MU[ k,j ] = MU[ j,k ]
