...
def get_Globals():
    ...
    HAM = "PF" # "AD", "PF" -- Solves the Asympotically Decoupled (AD) or Pauli-Fierz Hamiltonians
    ...
    Nf  = 200  # Only used if calculating result in Fock basis
...
def get_H_Total_Fock_Real__Pauli_Fierz__Wrapper( g_wc ):
    ...
    def compute_photon_number_PF_basis( N_PF, U, op_b_Total ):
    ...
    N_PF = np.zeros(( len(E) ))
    op_b = get_b()
    op_b_Total = kron( np.identity( Nmatter ), op_b.T @ op_b )
    N_PF = compute_photon_number_PF_basis( N_PF, U, op_b_Total  )
